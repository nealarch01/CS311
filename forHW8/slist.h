...
class slist : public llist { // inherit from the llist class
   public:
    ...
    el_t search2(el_t);
    // Search through the entire list if the node contains el_t key
    // If the key exists, returns the key back, if key does not exist, we return an uninitialized / blank el_t item
