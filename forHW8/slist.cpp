...
// Purpose: Another search function but this one returns the key if it was found
// Parameters: el_t object we want to find in the list
el_t slist::search2(el_t key) {
    Node *tempPtr = Front;
    while(tempPtr != NULL) { // sequentially search the entire list
        if(tempPtr->Elem == key) { // if the el_t object in the list is equal to el_t obj key, return the key
            return tempPtr->Elem; // return the object in the list that contains the key 
        }
        tempPtr = tempPtr->Next;
    }
    el_t blankData; // this means we did not found the key
    return blankData; // return a blank el_t object
}