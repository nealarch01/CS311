...
#include <iostream>
#include "stack.h"
using namespace std; // moving namespace below libraries because of implicit compile warnings
...
void stack::push(el_t elem) { 
   if (isFull()) throw StackOverflow_Error();
   else el[++top] = elem;
}
...
{ if (isEmpty()) throw StackUnderflow_Error();
  else elem = el[top--];
}
...
void stack::displayAll() {  
   ...
   else for(int i = top; i >= 0; i--) cout << el[i] << endl;
   cout << "--------------" << endl;
}
...
// it is empty 
void stack::clearIt() {
   el_t temp;  // use to temporarily store the returned value while popping the stack
   while(!isEmpty()) pop(temp); // no need to try catch block since the loop will not run if the stack is already empty
}